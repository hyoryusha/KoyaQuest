...
class NyonindoGameScene: SKScene {
    ...
    func spawnPilgrim() {
        ...
        for index in 1...numImages {
            let pilgrimName = "pilgrim_\(index)"
        ...
        let pilgrimAction = SKAction.animate(
            with: pilgrimDancingFrames,
            timePerFrame: 0.4,
            resize: false,
            restore: true
        )
        ...
        path.move(to: CGPoint(
            x: size.width + pilgrim.size.width,
            y: -pilgrim.size.height)
        )
        // entry point:
        path.addLine(to: CGPoint(x: size.width, y: 0))
        // first curve:
        path.addLine(to: CGPoint(
            x: size.width * 0.1 - (pilgrim.size.width * 0.5),
            y: size.height * 0.45 - (pilgrim.size.height ))
        )
        // second curve:
        path.addLine(to: CGPoint(
            x: size.width * 0.9 + (pilgrim.size.width * 0.25),
            y: size.height * 0.55 - (pilgrim.size.height * 0.25))
        )
        // third curve:
        path.addLine(to: CGPoint(
            x: size.width * 0.12,
            y: size.height * 0.7 - (pilgrim.size.height * 0.25))
        )
        // fourth curve?:
        path.addLine(to: CGPoint(
            x: size.width * 0.75,
        ...
        // gate entrance
        path.addLine(to: (CGPoint(x: size.width * 0.5,
                                  y: size.height * 0.9))
        )

        let scaleDown = SKAction.scale(by: 0.75, duration: 20)
        let followPath = SKAction.follow(path, asOffset: false, orientToPath: false, speed: 90.0)
    ...
    func showPostRoundFeedback() {
        ...
        let rikishiToCenterAnimation = SKAction.move(
            to: CGPoint(
                x: size.width * 0.5,
                y: size.height / 2 + (rikishi.size.height / 3 )),
            duration: 1.0
        )
