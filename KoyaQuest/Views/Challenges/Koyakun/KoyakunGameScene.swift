...
class KoyakunGameScene: SKScene {
    ...
    // static var koyaGameState: KoyakunGameState = .generatingKoyakun
    static var koyaGameState: KoyakunGameState = .none // this is how it is set up in the tutorial
    // did move to view -> setup -> perform initial generation -> game state changes to .gengerateKoyakun
    // how does this affect NotificationCenter in line 53 below?
    ...
    override func didMove(to view: SKView) {
        ...
        pointsDisplay.fontColor = UIColor(red: 0, green: 204 / 255, blue: 131 / 255, alpha: 1.0)
...
//            KoyakunGameScene.koyaGameState = .generatingKoyakun
        ...
        func handleGameOver() { // actions to be performed when the timer is out (on non-practice round)
            gameOver = true
        ...
        @objc func generateKoyakun() { // exposed to objc because it is called in the notification center selector
            guard let sceneView = self.view as? ARSKView else {return}
...
// 0.05 = 5 cm
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let name = touchedNodes.last?.name {
            ...
            if name == "exitButton" { // if the start game button is tapped, we need to transition to the game scene:
